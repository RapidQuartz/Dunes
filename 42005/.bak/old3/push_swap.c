...
/*   Updated: 2025/03/18 14:36:26 by akjoerse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
short		find_promising(t_ree *root);
...
t_snap *push_swap(t_snap **stack)
{
	...
	while (j++ < 420 && ++i && root->moves[0] == NULL)
	{
		if (root->moves[i] != NULL)
		{
			stump = branch_tree(root->moves[i]->tree);
			compare_branch(stump);
		}
		if (i == 11)
			i = 0;
	}
...
short		find_promising(t_ree *root)
{
	...
	if (root->order != NULL)
	{
		...
		while (i++ < 11)
		{
			if (root->moves[i] != NULL && root->moves[i]->delta < m)
			{
				j = i;
				m = root->moves[i]->delta;
			}
		}
	}
	return (j);
